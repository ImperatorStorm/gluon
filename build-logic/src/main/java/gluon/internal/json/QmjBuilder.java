package gluon.internal.json;

import java.io.IOException;
import java.nio.file.Path;

import gluon.internal.MinecraftVersion;
...
public final class QmjBuilder {
	public static void buildQmj(Project project, String version, String loaderVersion, MinecraftVersion minecraftVersion, GluonModuleExtensionImpl ext, Path path) throws IOException {
		...
		for (QslLibraryDependency depend : ext.getModuleDependencyDefinitions()) {
			for (QslLibraryDependency.ModuleDependencyInfo moduleDependencyInfo : depend.getDependencyInfo().get()) {
				...
				GluonModuleExtension depExt = depProject.getExtensions().getByType(GluonModuleExtension.class);
				writer.beginObject()
		...
		if (!ext.getEntrypoints().isEmpty()) {
			...
			for (GluonModuleExtensionImpl.NamedWriteOnlyList entrypoint : ext.getEntrypoints()) {
		...
		if (!ext.getInjectedInterfaces().isEmpty()) {
			...
			for (GluonModuleExtensionImpl.NamedWriteOnlyList injectedInterface : ext.getInjectedInterfaces()) {
