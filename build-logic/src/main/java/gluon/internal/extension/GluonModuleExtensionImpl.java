...
import gluon.internal.dependency.QslLibraryDependency;
import gluon.internal.json.Environment;
...
public class GluonModuleExtensionImpl extends GluonExtension implements GluonModuleExtension, Serializable {
	@Serial
	...
	@Inject
	public GluonModuleExtensionImpl(ObjectFactory factory, Project project) {
		super(project);
	...
	public String getLibrary() {
		return this.project.getParent().getExtensions().getByType(GluonLibraryExtension.class).getLibraryName().get();
	}
	...
	/**
	 * This method traverses up the internal Gluon module dependency tree, adding the different modules as either runtime or implementation only based on the dependency type for the module.
	 *
	...
	private void addTransitiveImplementations(QslLibraryDependency library, QslLibraryDependency.ModuleDependencyInfo info) {
		...
		GluonModuleExtensionImpl gluonModuleExtension = (GluonModuleExtensionImpl) depProject.getExtensions().getByType(GluonModuleExtension.class);

		for (QslLibraryDependency depLibrary : gluonModuleExtension.getModuleDependencyDefinitions()) {
